...
const Dashboard = () => {
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    try {
      const res = await axios.patch(
        `${RESTAURANT_API_END_POINT}/order/status/${orderId}`,
        { status: newStatus },
      ...
      if (res.data.success) {
        ...
        setDependency((prev) => !prev); // re-fetch if needed
      }
    } catch (error) {
  ...
  // safeguard for empty or invalid orders
  ...
  return (
    <div className="w-[90%] mx-auto h-[70vh]">
      <h2 className="text-5xl mt-4 text-center font-bold text-myColor opacity-50 pb-6 border-b-myColor border-b-2 rounded-3xl">
        Orders Overview
      </h2>
        ...
        {orders.map((order: any, index: number) => (
          <div key={index} className="p-4 w-92 shadow-xl rounded-xl border-t-6 border-myColor">
            ...
            <p className="text-md font-bold my-2">
              Address: <span className="text-sm font-medium">{order.deliveryInfo.address}</span>
            </p>
            <p className="text-md font-bold my-2">
              City: <span className="text-sm font-medium">{order.deliveryInfo.city}</span>
            </p>
            <p className="text-md font-bold my-2">
              Total Amount: <span className="text-sm font-medium">${order.totalAmount}</span>
            </p>

            {order?.cartItems?.map((item: any, idx: number) => (
              <div
                className="flex justify-between items-center gap-2 border-b-1 border-b-gray-400"
                key={idx}
              >
                <div className="flex items-center gap-3">
                  <img
                    className="w-10 h-10 rounded-full object-cover my-3"
                    src={item?.foodImage}
                    alt=""
                  />
                  <p className="font-semibold">{item?.foodName}</p>
                </div>
                <p className="font-bold text-myColor text-right">
            ...
            <form className="mt-3">
              <label className="text-md font-bold mt-3">Order Status:</label>
              <br />
              <select
